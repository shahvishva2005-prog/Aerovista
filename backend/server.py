...
async def admin_export(kind: str, fmt: str, user=Depends(require_roles("admin"))):
    if fmt not in ("csv", "xlsx"): raise HTTPException(status_code=400, detail="Format error")
    rows = [{"info": "Feature Ready"}]
    df = _df_from_records(rows, list(rows[0].keys()))
    return _stream_csv(df, kind) if fmt == "csv" else _stream_xlsx(df, kind)
