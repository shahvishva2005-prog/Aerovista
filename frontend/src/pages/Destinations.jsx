    ...
    <PageShell title="Destinations" subtitle="120+ destinations across 6 continents." testId="destinations-page"
      ...
      <div className="relative max-w-md mb-8">
        ...
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search city, country, IATA…"
          ...
          className="w-full bg-white/5 border border-white/10 rounded-full pl-11 pr-4 py-3 text-white text-sm focus:border-amber-400 outline-none" />
      </div>

      <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        {filtered.map((a) => (
          <div key={a.iata} className="glass-light rounded-xl p-4 hover:border-amber-400/40 border border-white/10 transition" data-testid={`dest-list-${a.iata}`}>
            <div className="flex items-center justify-between">
              <div>
                <div className="text-white font-medium">{a.city}</div>
                <div className="text-white/55 text-xs">{a.country}</div>
              </div>
              <div className="font-mono-aero text-amber-400 text-sm">{a.iata}</div>
            </div>
            <div className="text-white/40 text-xs mt-2 truncate">{a.name}</div>
          </div>
        ))}
      </div>
    </PageShell>
